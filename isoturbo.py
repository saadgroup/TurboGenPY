...
#  szm = -sin(theta)*cos(phi)*cos(alfa)-sin(theta)*sin(phi)*sin(alfa)
  ...
  if enableIO:
    ...
    writeufile = mp.Process(target=writefile, args=('u_'+str(nx)+'.txt','x',nxt,nyt,nzt,dx,dy,dz,patches,uarrays, fileformat))
    writeufile.start()
    
    writevfile = mp.Process(target=writefile, args=('v_'+str(nx)+'.txt','y',nxt,nyt,nzt,dx,dy,dz,patches,varrays, fileformat))
    writevfile.start()
    
    writewfile = mp.Process(target=writefile, args=('w_'+str(nx)+'.txt','z',nxt,nyt,nzt,dx,dy,dz,patches,warrays, fileformat))
    writewfile.start()
